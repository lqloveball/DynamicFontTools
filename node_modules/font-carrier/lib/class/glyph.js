...
var Glyph = Base.extend({
    ...
    /**
     * 获取经过_.clone与svgpath处理过后的 this.options.d
     * @param  {[type]} path    [导出路径]
     * @param  {[type]} options [description]
     * @return {[type]}         [description]
     */
    toAnalysisPath: function(path, options) {
        var data = _.clone(this.options);
        var ascent = this.__font.__fontface.get('ascent');
        var svgStr = '';
     ...
     * @return {[type]}         [description]
     */
    toOriginalPath: function(path, options) {
        // var data = _.clone(this.options);
        // var d = svgpath(this.options.d).translate(0, 0).scale(1, -1)
        if (options.path) {
            path = _path.resolve(process.cwd(), path);
            fs.writeFileSync(path, this.options.d);
        }
